...
def pick_panel_channel(guild: discord.Guild):
    priority = ["관전-봇", "관전", "관전-패널", "spectate", "observer", "general", "일반"]
    me = guild.me
...
async def ensure_panel_once(guild: discord.Guild):
    ...
    embed = discord.Embed(
        title=PANEL_TITLE,
        description="버튼으로 닉네임 앞에 `관전_`을 붙이거나 뗄 수 있어요.\n본인 닉네임만 변경됩니다."
    )
    await channel.send(embed=embed, view=SpectateView())
...
class SpectateView(discord.ui.View):
    ...
    # ===== 활성화 =====
    @discord.ui.button(label="활성화", style=discord.ButtonStyle.success, custom_id="spectate_on")
    async def activate(self, interaction: discord.Interaction, _):
        ...
        if member.nick == new_nick:
            ...
            await interaction.channel.send(
                f"{member.mention} 이미 `관전_` 접두사가 적용됨.",
    ...
    # ===== 비활성화 =====
    @discord.ui.button(label="비활성화", style=discord.ButtonStyle.danger, custom_id="spectate_off")
...
# ===== 관리자 명령 =====
@bot.tree.command(name="관전채널설정", description="관전 패널을 띄울 채널을 지정합니다.")
...
async def set_panel_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await interaction.response.send_message(
        f"✅ 이제 관전 패널은 {channel.mention} 에 표시됩니다.",
        ephemeral=True
    )
